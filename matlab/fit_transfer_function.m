...
% The current reference data was generated by a real, symmetric 129-tap
% FIR. Fit that model with complex least squares so both magnitude and phase
% participate in the fit. No System Identification Toolbox is required.
...
if mod(cfg.fit_taps, 2) ~= 1
    error('fit_taps must be odd for a Type-I symmetric FIR fit.');
end
...
%% Real, symmetric FIR complex least-squares fit
half_order = (cfg.fit_taps - 1) / 2;
fit_matrix = zeros(numel(omega), half_order + 1);
common_delay = exp(-1j * omega * half_order);
fit_matrix(:, 1) = common_delay;
for offset = 1:half_order
    fit_matrix(:, offset + 1) = ...
        2 * common_delay .* cos(omega * offset);
end
...
ridge = cfg.ridge_factor * trace(real_system.' * real_system) ...
    / size(real_system, 2);
unique_coefficients = (real_system.' * real_system ...
    + ridge * eye(size(real_system, 2))) ...
    \ (real_system.' * real_target);

fir_coefficients = zeros(cfg.fit_taps, 1);
fir_coefficients(half_order + 1) = unique_coefficients(1);
for offset = 1:half_order
    fir_coefficients(half_order + 1 - offset) = ...
        unique_coefficients(offset + 1);
    fir_coefficients(half_order + 1 + offset) = ...
        unique_coefficients(offset + 1);
end

h_fitted = fit_matrix * unique_coefficients;
...
fprintf('Model: real symmetric %d-tap FIR at %.0f Hz sample rate\n', ...
